...
# *In this location along the Madretschkanal, the undergrowth is dominated by Heracleum sphondylium in late summer*
...
# *Another large, easy to recognize plant is Clematis vitalba which grows all over the Suze river area.*
...
nt_tomap = near_threatened[['species_slug', 'place_slug', 'str_date', 'CH']].copy()
nt_locs = s_locs[s_locs.place.isin(nt_tomap.place_slug.unique())].copy()

nt_locs.set_index('place', inplace=True)
nt_map = nt_tomap.set_index('place_slug', inplace=True)

nt_map = pd.concat([nt_locs, nt_tomap], axis=1)
